...
<div class="max-w-2xl mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Editar Turno</h1>
    ...
    <div class="bg-white rounded-lg shadow p-8">
        <form action="{{ route('appointments.update', $appointment) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status" class="w-full px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 @error('status') border-red-500 @enderror">
                    ...
                    <option value="completed" {{ old('status', $appointment->status) === 'completed' ? 'selected' : '' }}>Completado</option>
                </select>
                @error('status')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" id="notes" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">{{ old('notes', $appointment->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
